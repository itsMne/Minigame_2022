...
HRESULT CreateTextureFromMemory(const uint8_t * wicData, size_t wicDataSize, ID3D11ShaderResourceView ** textureView, DirectX::TexMetadata * pTexInfo)
{
...
HRESULT CreateTextureFromFile(const wchar_t * szFileName, ID3D11ShaderResourceView ** textureView, DirectX::TexMetadata * pTexInfo)
{
	ID3D11Device* d3dDevice = GetDevice();
	*textureView = nullptr;
	TexMetadata meta;
...
HRESULT CreateTextureFromFile(const char * szFileName, ID3D11ShaderResourceView ** textureView, DirectX::TexMetadata * pTexInfo)
{
	if (*textureView)
	{
#ifdef USE_CONSOLE
		printf("ATTEMPTED TO CREATE：%s\n", szFileName);
		printf("WARNING: POINTER ALREADY CREATED\n", szFileName);
#endif
		return E_FAIL;
	}
	ID3D11Device* d3dDevice = GetDevice();
